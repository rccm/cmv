    ...
    dsz = xr.Dataset(
        {
            ...
            "ZonalDirectTrend_S": (["lat"], ts),
            "ZonalDirectTrend_P": (["lat"], tp),
...
if __name__ == "__main__":
    ...
    if resolution == 0.25:
            ...
            resname = '8'
    files = glob.glob("../../repack/Mission_"+dataname+'_HQ'+str(hq)+'_SD_'+resname+"KM*.nc")
    ...
    for r in range(n_ranks):
        if rank == r:
            ...
            for i in range(my_first, my_last):
                if i < numbers:
                    file = files[i]
                    outfilename = "".join('../../output/TrendingZonal_'+dataname+'_HQ'+str(hq)+'_SD_'+resname+'hKM_'+file.split('_')[5].split('.')[0]+'_bin.nc')
                    print(file.split('_')[5].split('.')[0])
